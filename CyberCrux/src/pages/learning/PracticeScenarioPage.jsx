...
export default function PracticeScenarioPage() {
  ...
  useEffect(() => {
    const fetchScenario = async () => {
      try {
        setLoading(true);
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/practice/scenarios/${id}`);
  ...
  const saveProgress = async (finalScore) => {
    ...
    try {
      ...
      const response = await fetch(import.meta.env.VITE_API_URL + '/api/practice/submit-answers', {
        method: 'POST',
        headers: {
