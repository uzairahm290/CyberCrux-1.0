...
const ToolPracticeGame = ({ toolName, onGameComplete, onClose }) => {
  ...
  const fetchScenarios = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/tool-practice/scenarios?tool_name=${toolName}`);
  ...
  const submitAnswer = async () => {
    ...
    try {
      const response = await fetch(import.meta.env.VITE_API_URL + '/api/tool-practice/check', {
        method: 'POST',
        headers: {
