...
export default function ProfileBrowser() {
  ...
  useEffect(() => {
    const fetchProfiles = async () => {
      try {
        ...
        // Fetch top 3 users
        const topResponse = await fetch(import.meta.env.VITE_API_URL + '/api/profiles?limit=3');
        ...
        const response = await fetch(import.meta.env.VITE_API_URL + '/api/profiles?limit=20&offset=3');
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/profiles?search=${encodeURIComponent(query)}&limit=20`);
      const data = await response.json();
