...
export default function ToolsAdmin() {
  ...
  const handleSearch = async () => {
    try {
      const res = await axios.get(import.meta.env.VITE_API_URL + "/api/tools/search", {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isEditing) {
        await axios.put(`${import.meta.env.VITE_API_URL}/api/tools/${editId}`, payload);
      } else {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/tools/${id}`);
      await loadTools();
    } catch (err) {
