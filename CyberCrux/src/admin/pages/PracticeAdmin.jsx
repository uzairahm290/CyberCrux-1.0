...
export default function PracticeAdmin() {
  ...
  const fetchScenarios = async () => {
    try {
      setLoading(true);
      const res = await fetch(import.meta.env.VITE_API_URL + "/api/practice/scenarios", {
        credentials: 'include'
      });
  ...
  const fetchCategories = async () => {
    try {
      const res = await fetch(import.meta.env.VITE_API_URL + "/api/practice/categories", {
        credentials: 'include'
      });
      const data = await res.json();
      if (data.success) {
  ...
  const fetchQuestions = async (scenarioId) => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/questions/scenario/${scenarioId}`, {
        credentials: 'include'
      });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this scenario? This action cannot be undone.")) {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/practice/scenarios/${id}`, {
          method: "DELETE",
          credentials: 'include'
        });
  ...
  const toggleActiveStatus = async (scenario) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/practice/scenarios/${scenario.id}`, {
        method: "PUT",
        headers: {
