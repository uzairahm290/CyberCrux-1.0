...
CREATE TABLE badges (
    ...
    icon VARCHAR(500),
    badge_type VARCHAR CHECK (badge_type IN ('streak', 'completion', 'category', 'score', 'special', 'time_based')) NOT NULL,
    criteria JSON NOT NULL,
    points_reward INT DEFAULT 0,
    rarity VARCHAR CHECK (rarity IN ('common', 'rare', 'epic', 'legendary')) DEFAULT 'common',
...
CREATE TABLE tool_practice_scenarios (
    ...
    difficulty VARCHAR CHECK (difficulty IN ('Easy', 'Medium', 'Hard')) DEFAULT 'Easy',
...
CREATE TABLE learning_resources (
    ...
    type VARCHAR CHECK (type IN ('guide', 'course', 'tool', 'reference', 'practice')) NOT NULL,
    description TEXT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (scenario_id) REFERENCES practice_scenarios(id) ON DELETE CASCADE
);
...
CREATE TABLE user_notifications (
    id INT PRIMARY KEY SERIAL,
    user_id INT NOT NULL,
    type VARCHAR CHECK (type IN ('badge', 'scenario', 'achievement', 'system')) DEFAULT 'badge',
    title VARCHAR(255) NOT NULL,
...
SELECT 
    ...
    (EXTRACT(EPOCH FROM CURRENT_DATE)/86400 - EXTRACT(EPOCH FROM CAST(us.last_login_date AS DATE))/86400) AS days_since_last_login,
    (EXTRACT(EPOCH FROM CURRENT_DATE)/86400 - EXTRACT(EPOCH FROM CAST(us.last_practice_date AS DATE))/86400) AS days_since_last_practice
FROM users u
