...
export default function BadgesPage() {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const badgesResponse = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/badges', {
        credentials: 'include'
      });
      ...
      try {
        const authResponse = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/auth/me', {
          credentials: 'include'
        });
        ...
        if (authData.success && authData.user) {
          ...
          const userBadgesResponse = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/badges/user/${authData.user.id}`, {
            credentials: 'include'
          });
      ...
      if (!userBadgesData || !userBadgesData.success) {
        try {
          const userBadgesResponse = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/badges/user/me', {
            credentials: 'include'
          });
          userBadgesData = await userBadgesResponse.json();
