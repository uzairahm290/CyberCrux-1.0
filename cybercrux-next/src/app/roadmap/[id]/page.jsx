...
export default function RoadmapViewPage() {
  ...
  useEffect(() => {
    async function fetchRoadmap() {
      try {
        setLoading(true);
        const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/blogs/${id}`);
        ...
        try {
          await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/blogs/${id}/view`, {
            method: 'POST',
            headers: {
        ...
        {suggested.length > 0 && (
          <section className="mt-16">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {suggested.map((s) => (
                <Link href={`/roadmap/${s.id}`} key={s.id} className="block group">
