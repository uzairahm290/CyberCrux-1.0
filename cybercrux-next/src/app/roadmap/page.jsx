...
export default function RoadmapPage() {
  ...
  const handleViewRoadmap = async (id) => {
    try {
      await axios.post(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/blogs/${id}/view`);
    } catch (err) {
  ...
  useEffect(() => {
    const fetchRoadmaps = async () => {
      try {
        const res = await axios.get((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + "/api/blogs1");
