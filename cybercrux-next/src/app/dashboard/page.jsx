...
export default function DashboardPage() {
  ...
  const fetchStreakData = async () => {
    ...
    try {
      const response = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/streak/user-streak/${user.id}`);
  ...
  const fetchUserStats = async () => {
    ...
    try {
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/practice/stats', {
        credentials: 'include'
      });
      const data = await response.json();
  ...
  const fetchRecentBlogs = async () => {
    try {
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/blogs');
      const data = await response.json();
  ...
  const fetchUpcomingChallenges = async () => {
    try {
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/practice/scenarios');
      const data = await response.json();
  ...
  const recordLogin = async () => {
    ...
    try {
      await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/streak/record-login', {
        method: 'POST',
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 border border-white/20 shadow-2xl">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link href="/blogs" className="text-blue-400 hover:text-blue-300 text-sm font-medium transition-colors">
                  View All
                </Link>
              </div>
