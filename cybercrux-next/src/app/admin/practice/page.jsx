...
export default function PracticeAdmin() {
  ...
  const fetchScenarios = async () => {
    try {
      setLoading(true);
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + "/api/practice/scenarios", {
        credentials: 'include'
      });
  ...
  const fetchCategories = async () => {
    try {
      const res = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + "/api/practice/categories", {
        credentials: 'include'
      });
      const data = await res.json();
      if (data.success) {
  ...
  const handleAdd = () => {
    router.push("/admin/practice/new");
  };
  ...
  const handleEdit = (scenario) => {
    ...
    router.push(`/admin/practice/edit/${scenario.id}`, { state: { scenario } });
  };

  const handleView = (scenario) => {
    router.push(`/admin/practice/view/${scenario.id}`, { state: { scenario } });
  };
  ...
  const fetchQuestions = async (scenarioId) => {
    try {
      ...
      const response = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/questions/scenario/${scenarioId}`, {
        credentials: 'include'
      });
  ...
  const handleDeleteQuestion = async (questionId) => {
    if (window.confirm("Are you sure you want to delete this question? This action cannot be undone.")) {
      try {
        const response = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/questions/${questionId}`, {
          method: 'DELETE',
          credentials: 'include'
        });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this scenario? This action cannot be undone.")) {
      try {
        const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/practice/scenarios/${id}`, {
          method: "DELETE",
          credentials: 'include'
        });
  ...
  const toggleActiveStatus = async (scenario) => {
    try {
      const res = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/practice/scenarios/${scenario.id}`, {
        method: "PUT",
        headers: {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-hidden">
            ...
            <div className="p-6 overflow-y-auto max-h-[60vh]">
              ...
              <div className="mb-6">
                <button
                  onClick={() => router.push(`/admin/practice/${selectedScenario.id}/questions/new`)}
                    ...
                    <div key={question.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-2 ml-4">
                          <button
                            onClick={() => router.push(`/admin/practice/${selectedScenario.id}/questions/edit/${question.id}`, {
