...
export default function BooksAdmin() {
  ...
  const fetchBooks = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/books');
  ...
  async function handleDeleteBook(bookId) {
    ...
    try {
      const response = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/books/${bookId}`, {
        method: 'DELETE',
      });
