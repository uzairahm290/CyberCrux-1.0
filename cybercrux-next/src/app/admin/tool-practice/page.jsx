...
const ToolPracticeAdmin = () => {
  ...
  const fetchScenarios = async () => {
    try {
      setLoading(true);
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/tool-practice/scenarios?admin=true');
  ...
  const fetchTools = async () => {
    try {
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/tools');
      const data = await response.json();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = editingScenario 
        ? `${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/tool-practice/scenarios/${editingScenario.id}`
        : (process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/tool-practice/scenarios';
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/tool-practice/scenarios/${id}`, {
        method: 'DELETE'
      });
