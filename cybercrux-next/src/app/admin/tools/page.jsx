...
export default function ToolsAdmin() {
  ...
  const loadTools = async () => {
    try {
      const res = await axios.get((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + "/api/tools");
      setTools(res.data);
    } catch (err) {
  ...
  const handleSearch = async () => {
    try {
      const res = await axios.get((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + "/api/tools/search", {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (isEditing) {
        await axios.put(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/tools/${editId}`, payload);
      } else {
        await axios.post((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + "/api/tools", payload);
      }
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/tools/${id}`);
      await loadTools();
    } catch (err) {
