...
const BadgeAdmin = () => {
  ...
  const fetchBadges = async () => {
    try {
      setLoading(true);
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/badges', {
        credentials: 'include'
      });
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch((process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555') + '/api/admin/badges', {
        method: 'POST',
        headers: {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/admin/badges/${editingBadge}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleDelete = async (badgeId) => {
    ...
    try {
      const response = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5555')}/api/admin/badges/${badgeId}`, {
        method: 'DELETE',
        credentials: 'include'
      });
